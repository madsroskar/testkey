...
const KeyBox = ({ k }: KeyBoxProps) => (
  <Box borderWidth={2} border="solid black" borderRadius="10px">
...
const useKeyEvents = () => {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (!event.repeat) {
        setPressedKeys((pressedKeys: Array<KeyType>) => [
          ...pressedKeys,
          mapKey(event),
        ]);
      }
    ...
    const onKeyUp = (event: KeyboardEvent) => {
      setPressedKeys((pressedKeys: Array<KeyType>) =>
        pressedKeys.filter((p) => p.code !== event.code)
      );
    };
...
const DebugWarning = () =>
  DEBUG ? (
    <Text position="fixed" top={0} left={0} color="red.400">
      Debug is active
    </Text>
  ) : null;
  ...
  return (
    <>
      ...
      >
        ...
        <Link
          href="https://github.com/madsroskar/testkey"
          color="yellow"
          isExternal
        >
          GitHub
